...
# ## assume a faster acclimation to light
# df_damped <- df %>% 
#   mutate_at(vars(one_of(c("tc", "vpd", "co2", "fapar"))), dampen_vec, tau = 40) %>% 
#   mutate_at(vars(one_of(c("ppfd"))), dampen_vec, tau = 10)
...
df_tmp %>% 
  ...
  ggplot() +
  geom_line(aes(doy, Rd, color = Type)) +
  labs(title = "Dark respiration")
```

(add acclimating vs. instantaneous light response)
