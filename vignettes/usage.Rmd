...
We can create a data frame (in [tidyverse](https://www.tidyverse.org/) this is a tibble) and apply the `rpmode()` function to each row.
```{r}
library(dplyr)
library(purrr)
df <- tibble(
...
- `calc_ftemp_inst_vcmax()` calculates the instantaneous temperature response of $V_\text{cmax}$. Let's run the P-model for `tc = 10` (degrees C). The ratio of $V_\text{cmax}/V_\text{cmax25}$ should equal the instantaneous temperatre scaling function for $V_\text{cmax}$ at 10 degrees C (calculated by `calc_ftemp_inst_vcmax(10)`):
```{r}
out_pmodel <- rpmodel( 
