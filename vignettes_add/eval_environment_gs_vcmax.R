eval_environment_gs_vcmax <- function(kphio, beta, nsteps = 50, method_jmaxlim = "none", vcmax_start, gs_start, show = "chi", method_opt = "LC"){
  ...
  df_tc <- tibble(
    ...
    unnest( out_pmodel ) %>%
    mutate( out = purrr::pmap(dplyr::select(., kmm, gammastar, ns_star, ca, vpd), wrap_calc_optimal_gs_vcmax, beta, vcmax_start, gs_start, method_opt = method_opt) ) %>%
    unnest( out ) %>%
    mutate( aj_num = purrr::pmap(dplyr::select(., ppfd, ca, chi, gammastar), calc_aj, kphio = kphio)) %>%
