...
out_analytical_none <- rsofun::rpmodel(
  tc             = tc,
  vpd            = vpd,
  co2            = co2,
  elv            = elv,
...
out_analytical_jmaxlim <- rsofun::rpmodel(
  tc             = tc,
  vpd            = vpd,
  co2            = co2,
  elv            = elv,
...
```{r echo=FALSE}
gg_none <- eval_environment(
  calc_optimal_chi_num,
...
calc_optimal_jmax_num <- function( kmm, gammastar, ns_star, ca, vpd, beta, chi = NULL, fn_chi = NULL, ppfd, kphio, c_cost, fn_jmax){
  #-----------------------------------------------------------------------
  # Input:    - float, 'kmm' : Pa, Michaelis-Menten coeff.
  #           - float, 'ns_star'  : (unitless) viscosity correction factor for water
  #           - float, 'vpd' : Pa, vapor pressure deficit
...
Now, let's find the optimal $\chi$ numerically using above function with the same parameters as calculated inside and returned by the P-model.
```{r}
calc_optimal_jmax_num( 
  out_analytical_none$kmm, 
...
Create data frames where one variable is varied at a time, holding all others constant, and apply rpmodel.
```{r echo=FALSE}
gg_none <- eval_environment(calc_optimal_jmax_num, kphio = kphio, method_jmaxlim = "wang17", show = "assim")
...
Now do the same with the Jmax limitation using `method_jmaxlim = wang17` in the `rpmodel()` function call.
```{r}
gg_jmaxlim <- eval_environment(calc_optimal_chi_num, kphio = kphio, method_jmaxlim = "wang17", show = "assim")
```
...
<!-- varlist_ll <- wrap_calc_optimal_gs_vcmax_ll( ppfd, fapar, kphio, out_analytical_none$kmm, out_analytical_none$gammastar, out_analytical_none$ns_star, out_analytical_none$ca, vpd, beta, out_analytical_none$vcmax, out_analytical_none$gs ) -->
