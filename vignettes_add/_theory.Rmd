...
# ppfd <- 300          # mol/m2/d; Original setting
ppfd <- 130          # mol/m2/d, same as 1500 umol/m2/s
# ppfd <- 1500 * 10^-6 # mol/m2/s
vpd  <- 1000         # Pa
co2  <- 400          # ppm
elv  <- 0            # m.a.s.l.
fapar <- 1           # fraction

out_analytical_none <- rpmodel(
  tc             = tc,
  vpd            = vpd,
  co2            = co2,
  elv            = elv,
...
out_analytical_jmaxlim <- rpmodel(
  tc             = tc,
  vpd            = vpd,
  co2            = co2,
  elv            = elv,
...
- First (numerically) derive optimal $\chi$ based on the original criterion (as described in the previous section).
...
calc_optimal_jmax_num <- function( kmm, gammastar, ns_star, ca, vpd, chi = NULL, ppfd, kphio, beta, c_cost){
  ...
  if (is.null(chi)){
    ...
    out_optim_chi$value <- -out_optim_chi$value
  }
...
calc_optimal_gs_vcmax <- function( par, args, maximize=FALSE, return_all=FALSE, method_opt = "LC", cost_scalar = 0.001717135){
  ...
  if (return_all){
    return( tibble( vcmax_num = vcmax,
                    gs_num = gs,
                    ci_num = ci,
                    chi_num = ci / ca,
                    a_c_num = a_c,
                    cost_transp_num = cost_transp,
                    cost_vcmax_num = cost_vcmax,
                    net_assim_num = net_assim,
                    kmm_num = kmm,
                    gammastar_num = gammastar))
  } else {
    return( net_assim )
  }
...
print( paste( "Vcmax^P:", kphio * fapar * ppfd * (varlist$ci_num + varlist$kmm_num) / (varlist$ci_num + 2 * varlist$gammastar_num )) )
...
```{r}
source("calc_optimal_gs_vcmax_jmax.R")
varlist_ll <- calc_optimal_gs_vcmax_jmax(out_analytical_none$kmm,
                                         out_analytical_none$gammastar,
                                         out_analytical_none$ns_star,
                                         out_analytical_none$ca,
                                         vpd, ppfd, fapar, kphio, beta,
                                         c_cost,
                                         out_analytical_none$vcmax,
                                         out_analytical_none$gs,
                                         40,
                                         method_jmaxlim_inst = "farquhar89") # smith37 or farquhar89

## Invoke function with optimised Vcmax and gs (in out_optim$par), now returning all variables
print( paste("Optimal chi from P-model:", out_analytical_none$chi ) )
print( paste("Optimal chi from Vcmax-gs optimization:", jmax_chi_opt$chi_opt ) )
print( paste("Optimal chi from Vcmax-gs opt. with light limit.:", varlist_ll$chi_mine, "using Jmax limitation from:", varlist_ll$method_jmaxlim_inst) )
```
...
Create data frames where one variable is varied at a time, holding all others constant, and apply rpmodel.
```{r echo=FALSE}
source("eval_environment_jmax_mine.R")
...
gg_none <- eval_environment_jmax_mine(
  ...
  c_cost = c_cost / 4.0,
  method_jmaxlim = "wang17", 
  method_jmaxlim_inst = "smith37", # smith37 or farquhar89
  show = "jv",
  vcmax_start = out_analytical_none$vcmax, 
  gs_start = out_analytical_none$gs,
  jmax_start = 40
...
#### Considering leaf energy balance
```{r}
source("calc_optimal_tcleaf_vcmax_jmax.R")
