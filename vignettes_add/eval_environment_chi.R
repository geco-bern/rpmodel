eval_environment_chi <- function(kphio, nsteps = 50, method_jmaxlim = "none", show = "chi"){
  ...
  df_tc <- tibble(
    ...
    unnest( out_pmodel ) %>%
    mutate( chi_num = purrr::pmap(dplyr::select(., kmm, gammastar, ns_star, ca, vpd), calc_optimal_chi_num, beta = 146) ) %>%
    unnest( chi_num ) %>%
